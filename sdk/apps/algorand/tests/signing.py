...
"""Sign helper that auto-confirms every on-device review screen."""

from . import algorand_ext
from .input_flows import InputFlowConfirmAll


def sign(session, instance_id, address_n, transactions, **kwargs):
    """Sign `transactions`, auto-confirming every review screen."""
    with session.test_ctx as client:
        client.set_input_flow(InputFlowConfirmAll(client).get())
